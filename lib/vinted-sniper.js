...
const APPLE_BRAND_ID = "54661";
const CATALOG_IDS    = "3661";  // Vinted phone/Apple-phone category (used for iPhone searches only)
const STATUS_IDS     = "2,3,1,6";

// Vinted brand IDs by product type. Applied to every search so results are
// brand-pure — no cross-brand noise. To find an ID for a missing brand:
//   GET https://{vinted-domain}/api/v2/brands?query={brand-name}
// then pick the matching entry's numeric id.
const VINTED_BRAND_IDS = {
  iphone:      APPLE_BRAND_ID, // Apple
  ipad:        APPLE_BRAND_ID, // Apple
  mac:         APPLE_BRAND_ID, // Apple
  airpods:     APPLE_BRAND_ID, // Apple
  // playstation: "???",       // Sony   — look up via /api/v2/brands?query=Sony
  // console:     null,        // Mixed brands — keyword search is sufficient
};
...
function isIphoneProduct(target) {
  return String(target?.product || "iphone").toLowerCase().includes("iphone");
}

function resolveVintedBrandId(target) {
  const product = String(target?.product || "iphone").toLowerCase();
  return VINTED_BRAND_IDS[product] ?? null;
}
...
async function searchListings(cookie, { keyword, priceFrom = 0, priceTo = 800, brandIds = null, catalogIds = null }) {
  const params = {
    search_text: keyword,
  ...
  if (brandIds) params.brand_ids = brandIds;
  if (catalogIds) params.catalog_ids = catalogIds;
  ...
  if (!Array.isArray(response.data?.items)) {
    console.log(chalk.yellow(`[vinted] search "${keyword}" — unexpected response (status ${response.status}, no items array)`));
    return [];
  }
...
async function runTargetQuery(target, cookie) {
  ...
  const isIphone = isIphoneProduct(target);

  // For non-iPhone targets, maxPrice is the all-in budget; the loop accepts listings up to
  // listedCeiling(maxPrice) * LOWBALL_MAX_RATIO. Use that as the API ceiling so we don't
  // silently miss items priced above maxPrice but still within the lowball window.
  const baseMax = (target?.maxPrice != null && Number.isFinite(Number(target.maxPrice)))
    ? Number(target.maxPrice) : 800;
  const apiPriceTo = isIphone
    ? baseMax
    : Math.ceil(listedCeiling(baseMax) * LOWBALL_MAX_RATIO);

  let items;
  try {
    items = await searchListings(cookie, {
      keyword,
      priceFrom: (target?.minPrice != null && Number.isFinite(Number(target.minPrice))) ? Number(target.minPrice) : 0,
      priceTo: apiPriceTo,
      brandIds: resolveVintedBrandId(target),
      catalogIds: isIphone ? CATALOG_IDS : null,
    });
  } catch (err) {
    console.log(chalk.yellow(`  [vinted] "${keyword}" — search failed: ${err.message}`));
    return;
  }
  ...
  for (const item of items) {
    ...
    let modelKey = null, storageGb = null, maxAllIn;

    if (isIphone) {
      modelKey = detectModel(item.title || "");
      if (!modelKey) continue;
      storageGb = detectStorageGb(`${item.title || ""} ${item.description || ""}`);
      maxAllIn = resolveMaxBuy(modelKey, storageGb);
      if (maxAllIn === null) continue;
    } else {
      const targetMax = Number.isFinite(Number(target?.maxPrice)) ? Number(target.maxPrice) : null;
      if (targetMax === null) continue;
      maxAllIn = targetMax;
    }
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const batteryPct = isIphone ? extractBatteryHealth(detail?.description || "") : null;
    ...
    const modelLabel = isIphone
      ? modelKey.split(" ").map((w) => w === "iphone" ? "iPhone" : w.charAt(0).toUpperCase() + w.slice(1)).join(" ")
      : (item.title || keyword);
    const storageLbl = isIphone && storageGb ? (storageGb >= 1024 ? `${storageGb / 1024}TB` : `${storageGb}GB`) : "";
    ...
    const record = {
      ...
      model: modelLabel,
      storage_gb: isIphone ? storageGb : null,
