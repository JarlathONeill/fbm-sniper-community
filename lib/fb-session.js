...
export async function bootstrapSession(proxyUrl = null, bootstrapLocation = null) {
  ...
  cdp.on('Network.requestWillBeSent', (event) => {
    ...
    try {
    // ─── Phase 1: Load city-slug search page (public, no login required) ──────

    // City-slug URLs (/marketplace/<city>/search/) are served by Facebook
    // without login in most regions, avoiding the login wall that blocks the
    // generic /marketplace/ page. The captured browse_request_params will
    // contain the city's marketplace_seo_page_id, but fb-scraper.js nulls
    // those fields and substitutes the user's real lat/lng before every
    // search, so geolocation stays correct.
    const citySlug = config.bootstrapCitySlug || 'london';
    const bootstrapUrl = `https://www.facebook.com/marketplace/${citySlug}/search/?query=iphone&sortBy=creation_time_descend`;
    console.log(`[fb-session] Loading city-slug search page: ${bootstrapUrl}`);
    await page.goto(bootstrapUrl, { waitUntil: 'networkidle2', timeout: 45000 });
